# OSC Tracking — IMU x Dual WebCam Sensor Fusion for VR
...
OSC対応IMUトラッカーと2台のWebカメラ（MediaPipe Pose Landmarker）をセンサーフュージョンし、VRフルボディトラッキングの精度と安定性を大幅に向上させるミドルウェア。

**対応トラッカー:** HaritoraX2（SlimeTora経由） — OSC出力できるIMUトラッカーなら原理的に対応可能（SlimeVR、Tundra等は今後対応予定）
...
- **トラッカー非依存設計**: OSC入力で動作するため、HaritoraX2以外のIMUトラッカーにも原理的に対応可能

## 必要環境

- Python 3.10+
- OSC出力対応のIMUトラッカー（現在の推奨: HaritoraX2 + [SlimeTora](https://github.com/OCSYT/SlimeTora) + [SlimeVR Server](https://github.com/SlimeVR/SlimeVR-Server)）
- Webカメラ x 2
- Windows（VRChat推奨）
...
- **DDD + Event駆動**: `domain/` (Skeleton Aggregate + Value Objects + Events), `application/` (EventBus), `persistence/` (Repository Protocol) の層構成。FusionEngine が `FrameProcessed` / `TrackingModeChanged` / `OcclusionDetected` 等を publish、各 subsystem が subscribe。新しい出力先や UI は subscriber を足すだけで追加可能。詳細は `CLAUDE.md` の Architecture セクション参照
